...
public class SurfaceVanillaBeach extends SurfaceBase
{
	...
	{
		super(config, Blocks.DIRT.getDefaultState(), Blocks.DIRT.getDefaultState());
            ...
            else if(b == Blocks.STONE.getDefaultState())
            {
            	...
            	if(cliff)
            	{
            		...
            		else
            		{
	            		...
	            		{
	            			if (rand.nextInt(3) == 0) {
	            				primer.setBlockState(MathUtils.globalToLocal(x), k, MathUtils.globalToLocal(y), cliffBlock2);
	            			}
	            			else {
	            				primer.setBlockState(MathUtils.globalToLocal(x), k, MathUtils.globalToLocal(y), cliffBlock1);
	            			}
            	...
            	else if(depth < 6)
            	{
	        		...
	        		else if(!dirt)
	        		{
                        if ( k > 56 && k < 68) { // one lower for under sand and 4 deeper
                        	...
                        	primer.setBlockState(MathUtils.globalToLocal(x), k, MathUtils.globalToLocal(y), Blocks.STONE.getDefaultState());
                        }
	        		}
            	}
            }
