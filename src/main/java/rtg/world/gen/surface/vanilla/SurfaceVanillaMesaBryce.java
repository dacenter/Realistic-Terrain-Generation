	...
	public void paintTerrain(ChunkPrimer primer, int i, int j, int x, int y, int depth, World world, Random rand, OpenSimplexNoise simplex, CellNoise cell, float[] noise, float river, Biome[] base)
	{
        ...
        for (int k = 255; k > -1; k--) {
            ...
            if (b == Blocks.AIR.getDefaultState()) {
                ...
                if (depth > -1 && depth < 12) {
                    if (cliff) {
	        			primer.setBlockState(MathUtils.globalToLocal(x), k, MathUtils.globalToLocal(y), CanyonColour.MESA_BRYCE.getBlockForHeight(i, effectiveHeight,j).getStateFromMeta(CanyonColour.MESA_BRYCE.getMetaForHeight(i, effectiveHeight,j)));
                    } else {
                        if (depth > 4) {
    	        			primer.setBlockState(MathUtils.globalToLocal(x), k, MathUtils.globalToLocal(y), CanyonColour.MESA_BRYCE.getBlockForHeight(i, effectiveHeight,j).getStateFromMeta(CanyonColour.MESA_BRYCE.getMetaForHeight(i, effectiveHeight,j)));
